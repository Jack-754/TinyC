...
direct_declarator:
    ...
    | direct_declarator LBP STATIC type_qualifier_list_opt assignment_expression RBP    { $$ = create_node("direct_declarator"); add_child($$, $1); add_child($$, $2); add_child($$, $3); add_child($$, $4); add_child($$, $5); add_child($$,$6);}
    | direct_declarator LBP type_qualifier_list STATIC assignment_expression RBP        { $$ = create_node("direct_declarator"); add_child($$, $1); add_child($$, $2); add_child($$, $3); add_child($$, $4); add_child($$, $5); add_child($$,$6);}
    | direct_declarator LBP type_qualifier_list_opt STAR RBP                            { $$ = create_node("direct_declarator"); add_child($$, $1); add_child($$, $2); add_child($$, $3); add_child($$, $4); add_child($$, $5);}
...
iteration_statement:
    ...
    | DO statement WHILE LP expression RP SEMICOLON                                             { $$ = create_node("iteration_statement"); add_child($$, $1); add_child($$, $2); add_child($$, $3); add_child($$, $4); add_child($$, $5); add_child($$, $6); add_child($$, $7); }
    | FOR LP expression_opt SEMICOLON expression_opt SEMICOLON expression_opt RP statement      { $$ = create_node("iteration_statement"); add_child($$, $1); add_child($$, $2); add_child($$, $3); add_child($$, $4); add_child($$, $5); add_child($$, $6); add_child($$, $7); add_child($$,$8); add_child($$,$9);}
    | FOR LP declaration expression_opt SEMICOLON expression_opt RP statement                   { $$ = create_node("iteration_statement"); add_child($$, $1); add_child($$, $2); add_child($$, $3); add_child($$, $4); add_child($$, $5); add_child($$, $6); add_child($$, $7); add_child($$,$8); }
    ;
